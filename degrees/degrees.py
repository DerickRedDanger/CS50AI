...
def shortest_path(source, target):
    ...
    #self.level was added to Node in util.py
    start = Node(source,False,False,neighbors_for_person(source),0)

    Queue=QueueFrontier()
    actors_explored=set()

    for i in start.action:
        if i[1] != target:
        ...
        for i in node.action:
            ...
            if i[1] != target:
                ...
                if i[1] not in actors_explored and node.level <= 5:
                    ...
                    child = Node(i[1],parent,i[0],neighbors_for_person(i[1]), (parent.level +1))
                    Queue.add(child)
                    actors_explored.add(i[1])
                ...
                x=0
                while x <= 6:
                    shortest.append((node.parent_movie,node.parent))
